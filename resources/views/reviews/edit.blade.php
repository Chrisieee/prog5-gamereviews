    ...
    <x-slot name="section">
        <form action="{{ route('reviews.update') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="mt-4 flex flex-col">
                ...
                <x-text-input id="rating" class="block mt-1 w-full" type="number" name="rating" min="1" max="5" :value="$review->rating" required/>
                <x-input-error :messages="$errors->get('rating')" class="mt-2"/>
            </div>

            <div class="mt-4 flex flex-col">
                <x-input-label for="image" :value="__('Image')"/>
                <x-text-input id="image" class="block mt-1 w-full" type="file" name="image" :value="$review->image" required/>
